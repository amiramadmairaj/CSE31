...
int has_cycle(node * head) {
  // Your code goes here:
  struct node * pointer1 = head, * pointer2 = head; //two pointers pointing to head

  while (pointer1 && pointer2 && pointer2 -> next) {
    pointer1 = pointer1 -> next; //increase by 1 (next element)
    pointer2 = pointer2 -> next -> next; //increase by 2 (next, next element)
    if (pointer1 == pointer2) //check if equal (if element is in list)
    {
      return 1; //true
    }
  }
